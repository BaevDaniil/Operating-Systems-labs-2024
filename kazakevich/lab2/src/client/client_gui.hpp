...
public:
    ...
    void addBookToClientList(const QString &bookName) const {
        if (bookName.isEmpty()) return;  // Защита от добавления пустых строк
...
private slots:
    void onGetBookClicked() {
        ...
        std::string str_bookName = bookName.toUtf8().constData();
    ...
    void onReturnBookClicked() {
        ...
        std::string bookName = selectedItem->text().toUtf8().constData();
